...
// Ícones disponíveis
const icons = ['🦴','❤️','🧬','💊','🧠','📚','⚗️','🔬','💉','🩺','🫀','🫁','👁️','🦷','💪','🧪','📖','✍️','🎓','💡','🔍','📊','📈','<img src="images/logo.png" alt="MedFocus" class="w-12 h-12 object-contain inline-block">','⚕️','🩹','🩸'];

// Estado Global
let currentUser=null, currentView='login', currentSubjectView=null, editingSubject=null;
let articles=[], users=[], notifications=[], subjects=[], permissions=[], posts=[], feedPermissions=[];
  ...
  <div class="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 flex items-center justify-center p-6">
    ...
    <div class="glass rounded-3xl shadow-modern-lg p-12 w-full max-w-lg border-2 border-white/30 fade-in">
      <div class="text-center mb-10">
        <div class="inline-flex items-center justify-center w-28 h-28 gradient-primary rounded-3xl mb-6 shadow-modern"><span class="text-6xl"><img src="images/logo.png" alt="MedFocus" class="w-12 h-12 object-contain inline-block"></span></div>
      ...
      <form id="login-form" class="space-y-6">
        <div><label class="block text-sm font-bold text-gray-700 mb-2 uppercase">${t('email')}</label>
          <input type="email" id="login-email" class="w-full px-5 py-4 bg-white/80 border-2 border-gray-200 rounded-xl focus:ring-4 focus:ring-purple-200 text-lg" required /></div>
        <div><label class="block text-sm font-bold text-gray-700 mb-2 uppercase">${t('password')}</label>
          <input type="password" id="login-password" class="w-full px-5 py-4 bg-white/80 border-2 border-gray-200 rounded-xl focus:ring-4 focus:ring-purple-200 text-lg" required /></div>
    ...
    <header class="glass border-b-2 border-white/30 shadow-modern">
      <div class="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
        <div class="flex items-center gap-4">
          <div class="w-12 h-12 gradient-primary rounded-xl flex items-center justify-center shadow-md"><span class="text-3xl"><img src="images/logo.png" alt="MedFocus" class="w-12 h-12 object-contain inline-block"></span></div>
          <h1 class="text-3xl font-black bg-gradient-to-r from-purple-600 to-blue-600 bg-clip-text text-transparent">MedFocus</h1>
        </div>
        <div class="flex items-center gap-4">
          ...
          ${currentUser?.role==='user'?`<button onclick="window.toggleNotifs()" class="relative p-3 bg-white rounded-xl hover:bg-gray-50 shadow-md"><span class="text-2xl">🔔</span>${unread>0?`<span class="notification-badge">${unread}</span>`:''}</button>`:''}
...
onAuthStateChanged(auth, async (user) => {
  if(user) {
    ...
    await Promise.all([loadUsers(), loadArticles(), loadNotifications(), loadSubjects(), loadPermissions(), loadPosts(), loadFeedPermissions()]);
...
// ============ SISTEMA DE PAGAMENTOS - ADICIONADO ============
...
const getAdminPayments = () => {
  ...
  <div class="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50">${getHeader()}${getAdminNav('payments')}
    <main class="max-w-6xl mx-auto px-6 py-10">
      <div class="glass rounded-3xl shadow-modern p-10 border-t-8 border-green-500">
        <h2 class="text-4xl font-black mb-8 bg-gradient-to-r from-green-600 to-emerald-600 bg-clip-text text-transparent">💳 ${t('managePayments')}</h2>
        <div class="space-y-6">${users.filter(u=>u.role!=='admin').map(u=>{
          ...
          const statusText = isPaid ? t('paid') : isOverdue ? t('overdue') : t('pending');
          ...
          return `
            <div class="payment-card ${isPaid?'paid':isOverdue?'overdue':''}">
              ...
              <div class="grid grid-cols-2 gap-4 mb-4">
                <div><p class="text-sm text-gray-600">${t('amount')}</p>
                  <p class="text-2xl font-bold text-gray-900">R$ ${payment.amount || '0,00'}</p></div>
                <div><p class="text-sm text-gray-600">${t('dueDate')}</p>
                  <p class="text-2xl font-bold text-gray-900">${payment.dueDate || '---'}</p></div>
              </div>
              <div class="flex gap-3">
                <button class="set-payment-btn flex-1 gradient-primary text-white py-3 px-4 rounded-xl font-bold" data-userid="${u.id}" data-username="${u.name}">⚙️ ${t('setDueDate')}</button>
                ${!isPaid?`<button class="register-payment-btn flex-1 gradient-success text-white py-3 px-4 rounded-xl font-bold" data-userid="${u.id}">💰 ${t('registerPayment')}</button>`:`<button class="mark-unpaid-btn flex-1 gradient-warning text-white py-3 px-4 rounded-xl font-bold" data-userid="${u.id}">↩️ Marcar Pendente</button>`}
              </div>
            </div>
          `;
        }).join('')}</div>
...
const getUserPayments = () => {
  ...
  const isOverdue = dueDate && now > dueDate && !isPaid;
  const statusBadge = isPaid ? 'badge-paid' : isOverdue ? 'badge-overdue' : 'badge-pending';
  const statusText = isPaid ? t('upToDate') : isOverdue ? t('overdue') : t('pending');
  const statusIcon = isPaid ? '🟢' : isOverdue ? '🔴' : '🟡';
  ...
  <div class="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50">${getHeader()}${getUserNav('payments')}
    <main class="max-w-4xl mx-auto px-6 py-10">
      <div class="glass rounded-3xl shadow-modern p-10 border-l-8 border-blue-500 mb-8">
        <h2 class="text-4xl font-black mb-8">💳 ${t('myPayments')}</h2>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          <div class="bg-white p-6 rounded-2xl shadow-md">
            <p class="text-sm text-gray-600 mb-2">${t('plan')}</p>
            <p class="text-3xl font-black text-gray-900">${t('monthly')}</p>
          </div>
          <div class="bg-white p-6 rounded-2xl shadow-md">
            <p class="text-sm text-gray-600 mb-2">${t('amount')}</p>
            <p class="text-3xl font-black text-gray-900">R$ ${payment.amount || '0,00'}</p>
          </div>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div class="bg-white p-6 rounded-2xl shadow-md">
            <p class="text-sm text-gray-600 mb-2">${t('dueDate')}</p>
            <p class="text-2xl font-bold text-gray-900">${payment.dueDate || '---'}</p>
          </div>
          <div class="bg-white p-6 rounded-2xl shadow-md">
            <p class="text-sm text-gray-600 mb-2">${t('status')}</p>
...
window.showSetPaymentModal = (userId, userName) => {
  ...
  modal.innerHTML = `
    <div class="glass rounded-3xl shadow-modern-lg p-8 max-w-md w-full">
      <div class="flex justify-between items-center mb-6">
        <h3 class="text-2xl font-black">${t('setDueDate')}: ${userName}</h3>
        <button onclick="this.closest('.fixed').remove()" class="text-4xl hover:text-red-600">×</button>
      </div>
      <div class="space-y-4">
        <div><label class="block text-sm font-bold mb-2">${t('amount')}</label>
        ...
        <div><label class="block text-sm font-bold mb-2">${t('dueDate')}</label>
          <input type="date" id="payment-due-date" class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl" value="${payment.dueDate || ''}" /></div>
      </div>
      <button id="save-payment-settings" data-userid="${userId}" class="w-full gradient-primary text-white py-4 rounded-xl font-bold text-lg shadow-md mt-6">💾 ${t('saveContent')}</button>
    </div>
  `;
  document.body.appendChild(modal);
...
// Adicionar ao attachEventListeners
...
// Adicionar loadPayments ao onAuthStateChanged
const originalAuth = onAuthStateChanged;
// Note: onAuthStateChanged já está no código, apenas adicionar loadPayments() ao array de loads
