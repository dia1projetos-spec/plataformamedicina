...
const getLoginView = () => `
  <div class="login-container">
    <div class="absolute top-4 right-4 flex gap-2" style="z-index:10;">
...
const getUserNav = (active) => {
  const items = [
    ...
    {id:'payments',icon:'💳',label:'Meus Pagamentos',view:'user-payments'},
...
const handleAddBook = async (e) => {
  ...
  try {
    console.log('📸 Processando capa:', coverFile.name);
    ...
    console.log('📄 PDF:', pdfFile.name, '-', pdfSizeMB, 'MB');
    
    // Check if PDF needs chunking (> 800KB = needs chunks)
    if(pdfFile.size > 819200) { // 800KB
      btn.textContent = '⏳ Dividindo PDF em partes...';
      
      // Read PDF as ArrayBuffer
      const pdfBuffer = await pdfFile.arrayBuffer();
      const pdfBase64Full = btoa(String.fromCharCode(...new Uint8Array(pdfBuffer)));
      
      // Split into chunks of 700KB (safe limit)
      const chunkSize = 700000; // ~700KB in base64
      const chunks = [];
      
      for(let i = 0; i < pdfBase64Full.length; i += chunkSize) {
        chunks.push(pdfBase64Full.slice(i, i + chunkSize));
      }
      ...
      for(let i = 0; i < chunks.length; i++) {
        btn.textContent = `⏳ Salvando parte ${i + 1}/${chunks.length}...`;
        await addDoc(collection(db, 'pdfChunks'), {
          bookId: docRef.id,
          chunkIndex: i,
          data: chunks[i]
        });
      ...
      console.log('✅ PDF salvo em chunks!');
    } else {
      // Small PDF - save normally
      ...
      console.log('✅ PDF salvo normalmente');
    }
    
    alert(`✅ SUCESSO!\n\nLivro "${title}" foi adicionado!\n📚 Categoria: ${category}\n📄 Tamanho: ${pdfSizeMB} MB`);
...
const handleDeleteBook = async (bookId) => {
  ...
  try {
    ...
    if(book && book.pdfChunks) {
      const chunksQuery = query(collection(db, 'pdfChunks'), where('bookId', '==', bookId));
      const chunksSnap = await getDocs(chunksQuery);
      
      for(const chunkDoc of chunksSnap.docs) {
        await deleteDoc(doc(db, 'pdfChunks', chunkDoc.id));
      }
      
      console.log(`✅ ${chunksSnap.size} chunks deletados`);
    }
...
const initPDFReader = async () => {
  ...
  // Check if PDF is in chunks
  if(currentBook.pdfChunks) {
    console.log(`📦 Carregando PDF em ${currentBook.pdfChunks} partes...`);
    ...
    chunksSnap.forEach(doc => {
      fullPdfBase64 += doc.data().data;
    });
    
    console.log('✅ PDF remontado!');
    pdfData = fullPdfBase64;
  } else {
    // Normal PDF (not chunked)
    pdfData = currentBook.pdf.split(',')[1];
  }
