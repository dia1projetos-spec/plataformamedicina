    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
      <div class="glass rounded-2xl sm:rounded-3xl shadow-modern p-4 sm:p-10 border-t-8 border-indigo-500 mb-6 sm:mb-8">
        ...
        <form id="add-book-form" class="space-y-4 sm:space-y-6 mb-8">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-bold mb-2">Capa do Livro (imagem)</label>
              <input type="file" accept="image/*" id="book-cover" class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl" required />
              <p class="text-xs text-green-600 mt-2">✨ Qualquer tamanho! Será comprimida automaticamente.</p>
            </div>
            <div>
              <label class="block text-sm font-bold mb-2">PDF do Livro</label>
              <input type="file" accept="application/pdf" id="book-pdf" class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl" required />
              <p class="text-xs text-green-600 mt-2">✨ Sem limite de tamanho!</p>
            </div>
          </div>
...
const handleAddBook = async (e) => {
  ...
  try {
    console.log('📸 Processando capa:', coverFile.name, '-', (coverFile.size / 1024 / 1024).toFixed(2), 'MB');
    
    // Compress and convert cover image
    const coverBase64 = await new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        ...
        img.onload = () => {
          ...
          console.log('✅ Capa comprimida:', (compressed.length / 1024 / 1024).toFixed(2), 'MB');
          resolve(compressed);
        };
        img.onerror = reject;
    ...
    console.log('📄 Processando PDF:', pdfFile.name, '-', (pdfFile.size / 1024 / 1024).toFixed(2), 'MB');
    btn.textContent = '⏳ Salvando PDF...';
    
    // Convert PDF to Base64 (no compression)
    ...
    // Success!
    alert('✅ SUCESSO!\n\nLivro "' + title + '" foi adicionado à biblioteca!\n\n📚 Categoria: ' + category);
